...
int timecount (int timeStart, HDC desk);
...
int main ()
    ...
    txCreateWindow (1900, 1100);

    HDC desk = txLoadImage ("_табло2.BMP");

    HDC altar = txLoadImage ("_алтарь3.BMP");

    HDC hegdehog = txLoadImage ("_ёж2.BMP");

    HDC bat = txLoadImage ("_летучая мышьтень.BMP");

    HDC rat = txLoadImage ("__крыса3.BMP");

    HDC solider = txLoadImage ("_солдаттень.BMP");

    HDC cat = txLoadImage ("_кот.BMP");

    HDC MagicAltar = txLoadImage ("_скатерть22.BMP");
...
void bushruns (HDC desk, HDC altar, HDC hegdehog, HDC bat, HDC rat, HDC solider, HDC cat, HDC MagicAltar)
   ...
   // Animal someCircle  =  {700,  700, 1, 12, 14, rat,     90,  53, 8, 10, 4, 6};
    ...
    Animal creatures [] = { {100,  100, 1, 0,  0, solider, 220, 220},
                            {600,  600, 1, 4,  6, hegdehog, 118, 82, 6, 8, 1, 3},
                            {300,  900, 1, 5,  7, bat,     135, 101, 6, 7, 2, 4},
                            {900,  300, 1, 12, 14, rat,     90,  53, 7, 9, 4, 6},
                            {700,  700, 1, 12, 14, rat,      90, 53, 8, 10, 4, 6} };
    double hearts = 8;
    int dt = 1;
...
//      txClear ();
        ...
        txBitBlt (txDC(), 0, 0, 1920, 1200, altar, 0, 0);

        txAlphaBlend (txDC(), 750, 425, 300, 285, MagicAltar);
       //                                                                                                                  k
        txSetFillColor (TX_LIGHTBLUE);
        //alphaBlend (blueCircle.x_, blueCircle.y_,140, 100, hegdehog, t/2%2 * txGetExtentX (hegdehog) / 2, 0, 118, 82, 1);


       // alphaBlend (600, 480, 800, 1000, cat, 0, 0, 800, 1000, 1);

        txSetFillColor (TX_RED);
        txSetColor (TX_BLACK);

        //alphaBlend (creatures [0].x_+5 + bush2.z_*5, bush2.y_+6 + bush2.z_*5, 200 * (bush2.z_/10.0), 200 * (bush2.z_/10.0), solider, 0,                                   220, 220, 220, 0.6);
        //alphaBlend (bush2.x_,                bush2.y_,                200 * (bush2.z_/10.0), 200 * (bush2.z_/10.0), solider, 0,   0, 220, 220, 1);
        ...
        //alphaBlend (redCircle.x_ + (redCircle.x_ - txGetExtentX()/2)/20, redCircle.y_, 135, 101, bat,  t/2%2 * txGetExtentX (bat) / 2, 101, 135, 101, 0.6 );
        //alphaBlend (redCircle.x_, redCircle.y_, 135, 101, bat, t/2%2 * txGetExtentX (bat) / 2, 0, 135, 101, amountOfGhostInAMouse );
        //alphaBlend (blueCircle.x_ + (blueCircle.x_ - txGetExtentX()/2)/20, blueCircle.y_, 135, 101, hegdehog,  t/2%2 * txGetExtentX (hegdehog) / 2, 101, 135, 101, 0.6 );
        ...
        //txCircle (yellowCircle.x_, yellowCircle.y_, 60); // невидимое
        ...
        //alphaBlend (greenCircle.x_, greenCircle.y_, 90, 53, rat, 0, 0, 90, 53, 1);

        //alphaBlend (someCircle.x_,  someCircle.y_,  90, 53, rat, 0, 0, 90, 53, 1);
...
void hummenmowe ()
    ...
    while (t <= 100)
         ...
         hummen (840 - 2.3*t, 260 , 1, 30, 0,t/3 %3 * 7, t/3 %3 *7, t/3 %4 *8, t/3 %4 *8, TX_RED);
...
void hummenmowey ()
    ...
    while (t <= 500)
        ...
        bloodrobenhood (435, 760, 1, 1, 0, 0, 0, t/10 %2 * 25, t/10 %2 * 25);
        hummen (610 , t + 2.3, 1, 30, 0, 35, 35, 0, 0, TX_RED);
        kaktus (1020, 585, 1, 0, 0);
        t++;
        txSleep (15);
...
void hummenmowex ()
    ...
    while (t <= 100)
        ...
        hummen (610 +2.3*t, 500 , 1, 30, 0, t/3 %3 *7, t/3 %3 *7, t/3 %4 *8, t/3 %4 *8, TX_RED);
        bloodrobenhood (435, 760, 1, 1, 0, 0, 0, t/10 %2 * 25, t/10 %2 * 25);
        kaktus (1020, 585, 1, 0, 0);
        t++;
        txSleep (25);
...
int  timecount (int timeStart, HDC desk)
    {
    txTransparentBlt (txDC(), 685, 850, 260, 110, desk, 0, 0, RGB (255, 0, 120));
    char str [100] = "";
...
void Animal::draw ()
    ...
    alphaBlend (x_ + (x_ - txGetExtentX()/2) * txGetExtentY () / 5000, y_ + y_ * txGetExtentY () / 5000, width_, height_,
                hdc_,  frame_/2%2 * txGetExtentX () / 2, txGetExtentY () / 2, width_, height_, 0.6 );

    alphaBlend (x_, y_, width_, height_, hdc_, frame_/2%2 * txGetExtentX () / 2, 0, width_, height_, 1);
